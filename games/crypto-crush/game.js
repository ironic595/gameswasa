...
export function init(container, args){
  ...
  const ICONS = ['₿','Ξ','₮','🐶','◎','Ł'];
  const COLORS = [
    {bg:'#FFB347', bd:'#FF8C00', light:'#FFD699'},
    {bg:'#7DD3FC', bd:'#0EA5E9', light:'#BAE6FD'},
    {bg:'#6EE7B7', bd:'#10B981', light:'#A7F3D0'},
    {bg:'#FDE047', bd:'#EAB308', light:'#FEF08A'},
    {bg:'#C4B5FD', bd:'#8B5CF6', light:'#DDD6FE'},
    {bg:'#FDA4AF', bd:'#F43F5E', light:'#FECDD3'}
  ];
  const SZ = 8;
...
.cc{width:100%;height:100%;min-height:100vh;display:flex;flex-direction:column;align-items:center;background:radial-gradient(ellipse at 50% 0%, #8B5CF6 0%, #6D28D9 25%, #4C1D95 60%, #1E0B3A 100%);color:#fff;font-family:Inter,system-ui;overflow:auto;position:relative;padding-bottom:24px}
...
.cc-stat{background:rgba(255,255,255,.95);border-radius:14px;padding:8px 12px;color:#2a1a5e;box-shadow:0 6px 16px rgba(0,0,0,.25)}
.cc-stat b{font-size:16px;font-weight:900;display:block}.cc-stat span{font-size:8px;letter-spacing:.08em;opacity:.6;font-weight:800;text-transform:uppercase}
.cc-stat.gold{background:linear-gradient(180deg,#FEF08A,#FACC15);border:2px solid #EAB308}.cc-stat.gold b{color:#000}
.cc-bar-wrap{width:100%;background:rgba(255,255,255,.92);border-radius:14px;padding:8px 12px;color:#2a1a5e}
.cc-bar{height:12px;background:#EDE9FE;border-radius:999px;overflow:hidden;margin-top:6px;border:1px solid #DDD6FE}.cc-bar-fill{height:100%;background:linear-gradient(90deg,#22C55E,#4ADE80);width:0%;transition:width .5s ease;border-radius:999px}
...
<div class="cc" id="root">
  ...
  async function trySwap(r1,c1,r2,c2){
    ...
    if(a.s==='color' || b.s==='color'){
      const br=a.s==='color'?r1:r2, bc=a.s==='color'?c1:c2;
    ...
    sel=null; await processCascade(found); busy=false;
  }
  ...
  function findMatches(){
    ...
    for(let r=0;r<SZ;r++){
      let c=0;
      while(c<SZ){
        const col=board[r][c]?.c; if(col===undefined){ c++; continue; }
        let end=c+1; while(end<SZ && board[r][end]?.c===col) end++;
        const len=end-c;
        if(len>=3){ const cells=[]; for(let k=c;k<end;k++) cells.push({r,c:k}); groups.push({cells, len, dir:'h', color:col}); cells.forEach(cell=>allCells.add(cell.r+','+cell.c)); }
        c=end;
      }
    }
    for(let c=0;c<SZ;c++){
      let r=0;
      while(r<SZ){
        const col=board[r][c]?.c; if(col===undefined){ r++; continue; }
        let end=r+1; while(end<SZ && board[end][c]?.c===col) end++;
        const len=end-r;
        if(len>=3){ const cells=[]; for(let k=r;k<end;k++) cells.push({r:k,c}); groups.push({cells, len, dir:'v', color:col}); cells.forEach(cell=>allCells.add(cell.r+','+cell.c)); }
        r=end;
      }
    }
  ...
  async function processCascade(firstFind){
    ...
    while(current.groups.length>0){
      ...
      current.groups.forEach(g=>{
        if(g.len===4){
          let target=g.cells[Math.floor(g.cells.length/2)];
          if(lastSwap){ if(g.cells.some(cell=>cell.r===lastSwap.r1 && cell.c===lastSwap.c1)) target={r:lastSwap.r1,c:lastSwap.c1}; else if(g.cells.some(cell=>cell.r===lastSwap.r2 && cell.c===lastSwap.c2)) target={r:lastSwap.r2,c:lastSwap.c2}; }
          if(!specialsToCreate.some(s=>s.r===target.r && s.c===target.c)) specialsToCreate.push({r:target.r,c:target.c,type: g.dir==='h' ? 'v' : 'h', color:g.color});
        } else if(g.len>=5){
          let target=g.cells[Math.floor(g.cells.length/2)];
          if(lastSwap){ if(g.cells.some(cell=>cell.r===lastSwap.r1 && cell.c===lastSwap.c1)) target={r:lastSwap.r1,c:lastSwap.c1}; else if(g.cells.some(cell=>cell.r===lastSwap.r2 && cell.c===lastSwap.c2)) target={r:lastSwap.r2,c:lastSwap.c2}; }
  ...
  async function processMatchesWithSet(keys, origin, combo=1, specialsToCreate=[]){
    ...
    score+=keys.length*10*combo*mult; crushes+=1;
    if(combo>1){ const comboEl=document.createElement('div'); comboEl.className='cc-combo'; comboEl.textContent=`COMBO x${combo}! +${keys.length*10*combo*mult}`; root.querySelector('#board').appendChild(comboEl); setTimeout(()=>comboEl.remove(),900); }
  ...
  const watcher=setInterval(()=>{ if(window.vrAd===4 && pendingAd){ const t=pendingAd; pendingAd=null; window.vrAd=0; window.vrAdType=null; (async()=>{
    if(t==='double'){ ui.innerHTML=`<div style="position:fixed;inset:0;background:rgba(0,0,0,.7);display:grid;place-items:center;z-index:40;color:white">Validando X2...</div>`; const res=await claim(true,true); if(res.ok){ totalReward+=BASE_REWARD*mult*2; crushes=0; startTime=Date.now(); startSession(); draw(); ui.innerHTML=`<div style="position:fixed;inset:0;background:rgba(0,0,0,.75);display:grid;place-items:center;z-index:40"><div style="background:#fff;border:3px solid #22C55E;border-radius:18px;padding:18px;text-align:center;width:min(320px,90vw)"><div style="color:#065F46;font-weight:900;font-size:18px">¡X2 +${fmt(BASE_REWARD*mult*2)} WASA!</div><button id="ok2" style="margin-top:10px;width:100%;height:44px;border-radius:999px;background:#22C55E;color:#000;font-weight:900;border:0">OK</button></div></div>`; ui.querySelector('#ok2').onclick=()=>{ ui.innerHTML=''; draw(); }; } else { ui.innerHTML=`<div style="position:fixed;inset:0;background:rgba(0,0,0,.75);display:grid;place-items:center;z-index:40"><div style="background:#fff;border:2px solid #ef4444;border-radius:12px;padding:12px;text-align:center;color:#991B1B"><div>Error: ${res.error||'server'}</div><button id="retry" style="margin-top:8px">Reintentar</button></div></div>`; ui.querySelector('#retry').onclick=()=>{ ui.innerHTML=''; showReward(); }; } }
  })(); } },150);
