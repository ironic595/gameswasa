// games/2048/game.js - v11 - v11 FINAL 0.0001 + PASS X5 REAL + SECURE SERVER
...
.w-stat{background:linear-gradient(180deg,#fef9c3,#fde68a);border:1.5px solid #5c4033;border-radius:12px;padding:8px 10px}
.w-stat b{font-size:14px;color:#3e2723;font-weight:900;display:block}
...
.w-stat.pass-active{background:linear-gradient(135deg,#A855F7,#7E22CE);border-color:#7E22CE;color:#fff}
.w-stat.pass-active b{color:#fff}
.w-stat.pass-active span{color:rgba(255,255,255,.8)}
...
.w-tile{position:absolute;border-radius:8px;display:grid;place-items:center;font-family:JetBrains Mono;font-weight:800;font-size:28px;transition:all.15s cubic-bezier(.34,1.56,.64,1);box-shadow:0 4px 12px rgba(0,0,0,.25);animation:pop.25s}
...
@media(max-width:820px){.w2048{overflow:auto}.w-layout{flex-direction:column;align-items:center;max-width:480px}.w-board-wrap{flex:0 0 auto;width:100%;max-width:480px}.w-side{max-width:480px;width:100%;min-width:0;position:static}.w-stats{grid-template-columns:1fr 1fr}}
</style>
<div class="w2048" id="wRoot">
  <div class="w-header"><div class="w-stats"><div class="w-stat"><b id="wScore">0</b><span>SCORE</span></div><div class="w-stat"><b id="wBest">0</b><span>BEST</span></div><div class="w-stat highlight" id="wRewardStat"><b id="wReward">+0.0000000</b><span id="wRewardLabel">WASA TOTAL</span></div><div class="w-stat"><b id="wCount">0</b><span>MERGES</span></div></div><div id="wPassBanner" style="display:none;background:linear-gradient(135deg,#A855F7,#7E22CE);color:#fff;border-radius:10px;padding:6px 10px;font-weight:900;font-size:11px;text-align:center">💎 WASA PASS X5 ACTIVO - Ganas x5</div></div>
  <div class="w-layout"><div class="w-board-wrap"><div class="w-grid" id="wGrid"></div><div class="w-tiles" id="wTiles"></div></div><div class="w-side"><div class="w-progress-wrap"><div style="display:flex;justify-content:space-between;font-size:10px;font-weight:900"><span>PROGRESO AD</span><span id="wProgText">0/40</span></div><div style="font-size:9px;opacity:.7;margin-top:2px" id="wProgHint">40 merges = 0,0001 + X2 AD</div><div class="w-progress"><div class="w-progress-bar" id="wProgBar"></div></div></div><div class="w-hint">🎮 <b>WASD / Flechas</b> PC<br>👆 <b>Swipe</b> celu</div></div></div><div id="wUI"></div></div>`;
  ...
  const MERGES_FOR_REWARD=40; const MERGES_FOR_FORCED=60; const BASE_REWARD=0.0001;
  ...
  let hasPass=false; let passChecked=false;

  function getReward(){ return BASE_REWARD * (hasPass?5:1); }
  ...
  function updatePassUI(){
    const banner=root.querySelector('#wPassBanner'); const stat=root.querySelector('#wRewardStat'); const label=root.querySelector('#wRewardLabel'); const hint=root.querySelector('#wProgHint');
    if(hasPass){
      if(banner) banner.style.display='block';
      if(stat){ stat.classList.add('pass-active'); }
      if(label) label.textContent='WASA TOTAL 💎 X5';
      if(hint) hint.textContent='40 merges = 0,0005 WASA (X5) + X2 AD = X10';
    } else {
      if(banner) banner.style.display='none';
      if(stat) stat.classList.remove('pass-active');
      if(label) label.textContent='WASA TOTAL';
      if(hint) hint.textContent='40 merges = 0,0001 + X2 AD • 💎 PASS X5 = 0,0005';
    }
  }

  async function startSession(){ currentSessionId=null; try{ const email=localStorage.getItem('wasa_email'); const wallet=localStorage.getItem('wasa_wallet'); const device_id=getDeviceId(); const r=await fetch(WORKER_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'start_game_session', email, wallet, device_id, game_slug:'2048', level:MERGES_FOR_REWARD})}); const j=await r.json(); if(j.ok){ currentSessionId=j.session_id; } }catch(e){} return currentSessionId; }
  async function claimSession(isDouble,adWatched){
    ...
    try{
      ...
      const r=await fetch(WORKER_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'claim_reward', session_id:currentSessionId, email, wallet, device_id, game_slug:'2048', level:MERGES_FOR_REWARD, ad_watched:adWatched, double_reward:isDouble, time_taken: elapsed})});
  ...
  function initGrid(){ grid=Array(4).fill(0).map(()=>Array(4).fill(0)); const gEl=root.querySelector('#wGrid'); gEl.innerHTML=''; for(let i=0;i<16;i++){ const c=document.createElement('div'); c.className='w-cell'; gEl.appendChild(c);} addRandom(); addRandom(); score=0; mergeCount=0; mergesSinceForced=0; lastWasDouble=true; totalReward=0; gameStartTime=Date.now(); checkPass().then(()=>{ updatePassUI(); }); startSession(); render(); ui.innerHTML=''; }
  ...
  function render(){ const tilesEl=root.querySelector('#wTiles'); if(!tilesEl) return; tilesEl.innerHTML=''; for(let r=0;r<4;r++) for(let c=0;c<4;c++){ const v=grid[r][c]; if(v===0) continue; const t=document.createElement('div'); t.className='w-tile t-'+(v>2048?'super':v>1024?1024:v); t.textContent=v; t.style.left='calc('+c+' * (25% + 2.5px))'; t.style.top='calc('+r+' * (25% + 2.5px))'; t.style.width='calc(25% - 7.5px)'; t.style.height='calc(25% - 7.5px)'; tilesEl.appendChild(t); } const set=(id,val)=>{ const el=root.querySelector('#'+id); if(el) el.textContent=val; }; set('wScore',score); set('wBest',Math.max(best,score)); set('wReward','+'+fmt(totalReward)); set('wCount',mergeCount); set('wProgText',mergeCount+'/'+MERGES_FOR_REWARD); const bar=root.querySelector('#wProgBar'); if(bar) bar.style.width=(mergeCount/MERGES_FOR_REWARD*100)+'%'; if(score>best){ best=score; localStorage.setItem('w2048_best',best); } }
  ...
  function showRewardModal(){
    const rew = getReward();
    const rewX2 = rew*2;
    const passBadge = hasPass ? `<span style="background:linear-gradient(135deg,#A855F7,#7E22CE);color:#fff;padding:2px 6px;border-radius:6px;font-size:9px;margin-left:6px">💎 PASS X5</span>` : `<div style="font-size:9px;opacity:.7;margin-top:4px"><a href="/wasa-pass.html" style="color:#7E22CE;font-weight:800;text-decoration:none">💎 Con PASS X5 sería ${fmt(BASE_REWARD*5)} → Comprar 5 USDT</a></div>`;
    const passExtra = hasPass ? `<div style="font-size:9px;opacity:.7;margin-top:2px">X2 AD = ${fmt(rewX2)} WASA (X10)</div>` : '';
    ui.innerHTML=`<div style="position:absolute;inset:0;background:rgba(62,39,35,.88);backdrop-filter:blur(16px);display:grid;place-items:center;z-index:20"><div style="background:linear-gradient(180deg,#fef9c3,#fde68a);border:2px solid #5c4033;border-radius:20px;padding:24px;text-align:center;width:min(360px,92vw)"><div style="font-size:32px">🪵🎉</div><div style="font-weight:900;margin:8px 0">¡40 COMBINACIONES!</div><div style="display:flex;flex-direction:column;align-items:center;background:rgba(34,197,94,.15);border:1px solid rgba(34,197,94,.4);border-radius:12px;padding:10px;margin:12px 0;font-weight:800;color:#14532d"><div>💰 +${fmt(rew)} WASA ${hasPass?'💎':''}</div>${hasPass?'':''}</div>${passBadge}${passExtra}<button id="btnDouble" style="width:100%;padding:12px;border-radius:12px;font-weight:800;border:2px solid #5c4033;background:linear-gradient(135deg,#fbbf24,#f59e0b);margin-top:10px">📺 X2 = ${fmt(rewX2)} WASA ${hasPass?'(X10)':''}</button><button id="btnClaim" style="width:100%;padding:12px;border-radius:12px;font-weight:800;border:2px solid #5c4033;background:#3e2723;color:#f5deb3;margin-top:8px">COBRAR ${fmt(rew)} WASA</button></div></div>`; ui.querySelector('#btnDouble').onclick=()=> openAd('double'); ui.querySelector('#btnClaim').onclick=async()=>{ const btn=ui.querySelector('#btnClaim'); if(btn){ btn.textContent='⏳ VALIDANDO...'; btn.disabled=true; } const res=await claimSession(false,false); if(res.ok){ totalReward+=res.reward||rew; mergeCount=0; lastWasDouble=false; gameStartTime=Date.now(); startSession(); render(); ui.innerHTML=`<div style="position:absolute;inset:0;background:rgba(0,0,0,.75);display:grid;place-items:center;z-index:20"><div style="background:#fef9c3;border:2px solid #22c55e;border-radius:12px;padding:12px;text-align:center;width:min(320px,92vw)"><div style="color:#22c55e;font-weight:900">¡+${fmt(res.reward||rew)} WASA! ${res.hasPass?'💎 X5':''}</div><button id="ok" style="margin-top:8px;width:100%;background:#22c55e;color:black;font-weight:900;padding:10px;border-radius:999px">OK</button></div></div>`; ui.querySelector('#ok').onclick=()=>{ ui.innerHTML=''; render(); if(!canMove()) showGameOverModal(); }; } else { if(btn){ btn.textContent=res.error||'REINTENTAR'; btn.disabled=false; } } }; }
  function move(dir){ const old=JSON.stringify(grid); let gained=0, merges=0; const proc=(arr)=>{ let a=arr.filter(x=>x!==0); for(let i=0;i<a.length-1;i++){ if(a[i]===a[i+1]){ a[i]*=2; a[i+1]=0; gained+=a[i]; merges++; } } a=a.filter(x=>x!==0); while(a.length<4) a.push(0); return a; }; const ng=grid.map(r=>[...r]); if(dir==='left'){ for(let r=0;r<4;r++) ng[r]=proc(ng[r]); } else if(dir==='right'){ for(let r=0;r<4;r++) ng[r]=proc([...ng[r]].reverse()).reverse(); } else if(dir==='up'){ for(let c=0;c<4;c++){ let col=[ng[0][c],ng[1][c],ng[2][c],ng[3][c]]; col=proc(col); for(let r=0;r<4;r++) ng[r][c]=col[r]; } } else if(dir==='down'){ for(let c=0;c<4;c++){ let col=[ng[0][c],ng[1][c],ng[2][c],ng[3][c]]; col=proc(col.reverse()).reverse(); for(let r=0;r<4;r++) ng[r][c]=col[r]; } } if(JSON.stringify(ng)!==old){ grid=ng; score+=gained; mergeCount+=merges; mergesSinceForced+=merges; addRandom(); render(); if(mergeCount>=MERGES_FOR_REWARD){ showRewardModal(); return; } if(mergesSinceForced>=MERGES_FOR_FORCED &&!lastWasDouble){ openAd('forced'); return; } if(!canMove()){ setTimeout(showGameOverModal, 200); return; } } else { if(!canMove()){ showGameOverModal(); } } }
  const watcher=setInterval(()=>{ if(window.vrAd===4 && _rewardPending){ const type=_rewardPending; _rewardPending=null; window.vrAd=0; window.vrAdType=null; window._gm_shown=false; (async()=>{ if(type==='double'){ ui.innerHTML=`<div style="position:absolute;inset:0;background:rgba(0,0,0,.75);display:grid;place-items:center;z-index:20"><div style="background:#fef9c3;border:2px solid #5c4033;border-radius:12px;padding:12px;text-align:center"><div style="font-weight:900">Validando X2...</div></div></div>`; const res=await claimSession(true,true); if(res.ok){ totalReward+=res.reward||getReward()*2; mergeCount=0; mergesSinceForced=0; lastWasDouble=true; gameStartTime=Date.now(); startSession(); render(); ui.innerHTML=`<div style="position:absolute;inset:0;background:rgba(0,0,0,.75);display:grid;place-items:center;z-index:20"><div style="background:#fef9c3;border:2px solid #22c55e;border-radius:12px;padding:12px;text-align:center;width:min(320px,92vw)"><div style="color:#22c55e;font-weight:900">¡X2 +${fmt(res.reward||getReward()*2)} WASA!</div><button id="ok2" style="margin-top:8px;width:100%;background:#22c55e;color:black;font-weight:900;padding:10px;border-radius:999px">OK</button></div></div>`; ui.querySelector('#ok2').onclick=()=>{ ui.innerHTML=''; render(); if(!canMove()) showGameOverModal(); }; } else{ ui.innerHTML=`<div style="position:absolute;inset:0;background:rgba(0,0,0,.75);display:grid;place-items:center;z-index:20"><div style="background:#fef9c3;border:2px solid #ef4444;border-radius:12px;padding:12px;text-align:center"><div style="color:#ef4444">Error X2: ${res.error||'server'}</div><button id="retry" style="margin-top:8px;width:100%;background:white;color:black;padding:8px;border-radius:999px">Reintentar</button></div></div>`; ui.querySelector('#retry').onclick=()=>{ ui.innerHTML=''; showRewardModal(); }; } } if(type==='forced'){ mergesSinceForced=0; ui.innerHTML=''; render(); if(!canMove()) showGameOverModal(); } })(); } },150);
  ...
  let sx=0,sy=0; root.addEventListener('touchstart',e=>{ sx=e.touches[0].clientX; sy=e.touches[0].clientY; },{passive:true}); root.addEventListener('touchend',e=>{ if(!sx) return; const dx=e.changedTouches[0].clientX-sx; const dy=e.changedTouches[0].clientY-sy; if(Math.abs(dx)>Math.abs(dy)){ if(Math.abs(dx)>30) move(dx>0?'right':'left'); } else{ if(Math.abs(dy)>30) move(dy>0?'down':'up'); } sx=0; sy=0; },{passive:true});
  initGrid();
