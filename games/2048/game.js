...
<div class="w2048" id="wRoot">
  ...
  async function claimSession(isDouble,adWatched){
    ...
    try{ const email=localStorage.getItem('wasa_email'); const wallet=localStorage.getItem('wasa_wallet'); const device_id=getDeviceId();
      const r=await fetch(WORKER_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'claim_reward', session_id:currentSessionId, email, wallet, device_id, game_slug:'2048-madera', level:MERGES_FOR_REWARD, ad_watched:adWatched, double_reward:isDouble, time_taken:MERGES_FOR_REWARD})});
  ...
  function move(dir){
    ...
    if(JSON.stringify(ng)!==old){
      grid=ng; score+=gained; mergeCount+=merges; mergesSinceForced+=merges;
      addRandom(); render();
