export function init(container, args){
  ...
  let enemies=[], roadOffset=0, particles=[], keys={l:false,r:false,up:false}, last=performance.now(), levelStart=0;
  ...
  function resetLevel(lvl){
    ...
    enemies=[]; particles=[]; roadOffset=0; spawnTimer=0;
  ...
  function spawnEnemy(y, initial){
    ...
    const x = left + lane*laneW + laneW/2 + (Math.random()-0.5)*laneW*0.2;
  ...
  function claimReward(){ const t=_rewardPending; _rewardPending=null; window.vrAd=0; window.vrAdType=null; window._gm_shown=false; if(t==='double' && !adClaimedThisLevel){ adClaimedThisLevel=true; const dbl=tempCoins*2; coins+=dbl; setCoins(coins); level++; saveMax(level); resetLevel(level); state='playing'; renderUI(); } else if(t==='revive'){ fuel=Math.min(100,fuel+50); player.invul=1.6; player.flash=0; state='playing'; renderUI(); } }
  ...
  function renderUI(){
    ...
    if(state==='reward_modal' && _rewardPending){ h+=`<div style="position:absolute;inset:0;z-index:20;background:rgba(0,0,0,0.9);display:flex;align-items:center;justify-content:center;pointer-events:auto;padding:12px"><div style="background:#101a2e;border:2px solid #22c55e;border-radius:12px;padding:16px;max-width:300px;width:100%;text-align:center"><div style="color:#22c55e;font-weight:900">${_rewardPending==='double'?'x2 RECOMPENSA!':'REVIVISTE!'}</div><div style="color:white;margin-top:6px">${_rewardPending==='double'?fmtWASA(tempCoins*2)+' $WASA':'50 FUEL'}</div><button id="claim" style="margin-top:10px;width:100%;background:#22c55e;color:black;font-weight:900;padding:11px;border-radius:8px">RECLAMAR</button></div></div>`; }
    ui.innerHTML=h;
    ...
    ui.querySelector('#next')?.addEventListener('click',()=>{ coins+=tempCoins; setCoins(coins); level++; saveMax(level); resetLevel(level); state='playing'; renderUI(); });
  ...
  let watcher=setInterval(()=>{ if(window.vrAd===4 && _rewardPending){ state='reward_modal'; renderUI(); } },120);
  let raf;
  function loop(now){
