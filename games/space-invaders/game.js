export function init(container, args){
  ...
  function createShip(level, index=0, total=1){
    ...
    return {x:dir===1?-120-index*150:W+120+index*150,y:yBase,w:cfg.w,h:cfg.h,hp:hpBase,maxHp:hpBase,dir,speed:cfg.speed+(level-1)*2,hitFlash:0,type:Math.floor((level-1)/10),cfg,ghostTimer:0,zigTimer:0,teleportTimer:0,originalY:yBase};
  }
  ...
  // === SISTEMA vrAd EXACTO COMO PEDISTE, sin triple click ===
  function openAd(type){
    if(window.vrAd!==0) return; // evita doble click
    console.log('[GAME] pide ad',type,'vrAd=1');
    _pendingAdType=type;
    _rewardPending=type;
    window.vrAdType=type;
    window.vrAd=1; // 1 = juego pide ad -> index ve 1 y pone 2 y muestra ad
    pauseBefore=state;
    state='paused'; // pausa visualmente, pero el loop real se congela por vrAd 1|2|3
    renderUI();
  }

  function claimReward(){
    const type=_rewardPending;
    console.log('[GAME] claimReward',type);
    _rewardPending=null;
    _pendingAdType=null;
    // IMPORTANTE: resetear vrAd a 0 SOLO despues de reclamar, con 1 click
    window.vrAd=0;
  ...
  function handleShoot(){
    ...
    // LOGICA ORIGINAL: despues de disparar, la barra vuelve a moverse (no se queda trabada)
    game.power.state='moving';
    game.power.dir = game.power.locked>=50 ? -1 : 1;
    renderUI();
  }

  // Watcher vrAd=4 -> recompensa, 1 solo modal, sin triple click
  setInterval(()=>{
    if(window.vrAd===4 && _rewardPending){
      console.log('[GAME] vrAd 4 detectado -> reward_modal');
      state='reward_modal';
      renderUI();
      // NO reseteamos vrAd aqui, lo resetea claimReward con 1 click
    }
  },200);

  let raf;
  function loop(now){
    // Si hay ad en curso 1|2|3, congelar TODO incluido power bar
    ...
    if(state==='playing'){
      ...
      // POWER BAR LOGICA EXACTA DE TU ARCHIVO ORIGINAL QUE FUNCIONA
    ...
    // UPDATE DIRECTO DE DOM DE POWER BAR COMO EN TU ARCHIVO ORIGINAL - no depende de renderUI
    if(state==='playing'){
