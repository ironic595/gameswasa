  ...
  let raf; function loop(now){
    ...
    if(state==='playing'){
      ...
      for(let i=game.bullets.length-1;i>=0;i--){
        ...
        if(hit) continue;
        if(b.y<-30||b.x<-30||b.x>W+30){game.bullets.splice(i,1);game.misses++;game.power.state='moving'; game.power.dir=game.power.locked>=50?-1:1; if(game.misses>=5){tempCoins=0; state='gameover';renderUI();}}
      }
    }
