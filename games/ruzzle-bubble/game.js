...
export function init(container, args){
  ...
  const COLOR_STAGES = [
    ...
    {name:'VERDE', light:'#dcfce7', dark:'#16a34a'},
    {name:'CELESTE', light:'#e0f2fe', dark:'#0284c7'},
    {name:'SALMON', light:'#ffe4e6', dark:'#f43f5e'},
    {name:'AMARILLO',light:'#fef9c3', dark:'#ca8a04'},
    {name:'NARANJA', light:'#ffedd5', dark:'#ea580c'},
    {name:'TURQUESA',light:'#ccfbf1', dark:'#0d9488'},
    {name:'ROSA', light:'#fce7f3', dark:'#db2777'},
  ];
  ...
  function getLevelColors(lvl){
    ...
    let progress = ((lvl-1)%10)/9;
    let bg = lerpColor(stage.light, stage.dark, progress);
    let darkBg = lerpColor(bg, '#08080d', 0.85 - progress*0.15);
  ...
  function applyLevelTheme(lvl){
    let {darkBg, accent} = getLevelColors(lvl);
    ...
    let top = container.querySelector('#pb6title'); if(top){ top.style.color = accent; }
    let dinoCard = container.querySelector('.dino-card'); if(dinoCard){ dinoCard.style.borderColor = accent+'55'; }
    let bar = container.querySelector('#pb6bar'); if(bar){ bar.style.background = `linear-gradient(90deg, ${accent}, ${accent}aa)`; }
  }
  ...
  function hm(e){ let seen=new Set(),qq=[]; for(let c=0;c<B-1;c++) if(e[0][c]!==null){ qq.push([0,c]); seen.add(`0,${c}`);} while(qq.length){let [r,c]=qq.shift();for(let [nr,nc] of ru(r,c)){if(nr<0||nr>=Q||nc<0||nc>=B)continue;if(nr%2===1 && nc>=B-1) continue; let k=`${nr},${nc}`;if(seen.has(k))continue;if(e[nr][nc]!==null){seen.add(k);qq.push([nr,nc]);}}} let floating=[];for(let r=0;r<Q;r++) for(let c=0;c<B;c++){if(r%2===1 && c>=B-1) continue; if(e[r][c]!==null&&!seen.has(`${r},${c}`))floating.push([r,c]);}return floating; }
...
@media(max-width:900px){.pb6-body{flex-direction:column;align-items:center;width:100%}.pb6-left{width:min(100vw - 16px, ${RN}px)}.pb6-right{width:min(100vw - 16px, 300px)}.pb6-canvas{width:100%!important;height:auto!important;aspect-ratio:${RN}/${NU}}}
  ...
  const ctx=canvas.getContext('2d',{alpha:false}); ctx.scale(1,1);
  let audioCtx=null;
  ...
  let rafId=null, isShooting=false, shotsSinceAd=0; const SHOTS_PER_AD=45;
  ...
  function loop(){ if(isShooting && shooting){ shooting.x+=shooting.dirX*14; shooting.y+=shooting.dirY*14; if(shooting.x<=WE/2+2||shooting.x>=RN-WE/2-2){ shooting.dirX*=-1; shooting.x=Math.max(WE/2+2,Math.min(RN-WE/2-2,shooting.x)); sfxBounce(); } if(shooting.y<=WE/2+10){ let p=lu(shooting.x,shooting.y,grid); if(p) place(p.r,p.c,shooting.col); else { isShooting=false; shooting=null; drawBoard(); } return; } for(let r=0;r<Q;r++) for(let c=0;c<B;c++){ if(r%2===1 && c>=B-1) continue; if(grid[r][c]===null)continue; let g=Oe(r,c); if(Math.hypot(g.x-shooting.x,g.y-shooting.y)<WE*0.9){ let p=lu(shooting.x,shooting.y,grid); if(p) place(p.r,p.c,shooting.col); else { isShooting=false; shooting=null; drawBoard(); } return; } } drawBoard(); rafId=requestAnimationFrame(loop); } }
  function place(r,c,col){ let pendingCol=col; isShooting=false; shooting=null; if(rafId){ cancelAnimationFrame(rafId); rafId=null; } if(r%2===1 && c>=B-1) c=B-2; if(grid[r][c]!==null){ let alt=lu(Oe(r,c).x,Oe(r,c).y,grid); if(!alt){ shot++; shotsSinceAd++; checkAd(); updateUI(); drawBoard(); return; } r=alt.r; c=alt.c; } let ng=tu(grid); ng[r][c]=pendingCol; if(r%2===1) ng[r][B-1]=null; if(!isConnectedTop(ng,r,c)){ ng[r][c]=null; shot++; shotsSinceAd++; checkAd(); updateUI(); grid=ng; drawBoard(); return; } let conn=vm(ng,r,c); if(conn.length>=3){ addPopAnim(r,c,pendingCol,conn.length,false); conn.forEach(([rr,cc])=> ng[rr][cc]=null ); let floating=hm(ng); if(floating.length>0) addPopAnim(floating[0][0],floating[0][1],pendingCol,floating.length,true); floating.forEach(([rr,cc])=> ng[rr][cc]=null ); popped+=conn.length+floating.length; score+=conn.length*15+floating.length*8; grid=ng; shot++; shotsSinceAd++; checkAd(); updateUI(); drawBoard(); if(popped>=target()||grid.flat().every(v=>v===null)){ win(); return; } } else { grid=ng; shot++; shotsSinceAd++; checkAd(); updateUI(); drawBoard(); } if(shot>=need()){ pushCeil(); ceilT=maxT(); shot=0; } cur=nxt; nxt=Math.floor(Math.random()*colors()); updateUI(); drawBoard(); if(grid[Q-1].some(v=>v!==null)) lose(); }
  function win(){
    ...
    container.querySelector('#bCl').onclick=async()=>{
      let b=container.querySelector('#bCl'); b.textContent='VALIDANDO...'; b.disabled=true;
