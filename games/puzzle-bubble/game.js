// game v16 - FIX GHOST BALL AFTER MATCH + ADS 30 SHOTS
...
@media(max-width:900px){.pb6-wrap{padding:8px}.pb6-body{flex-direction:column;align-items:center;width:100%}.pb6-left{width:min(100vw - 16px, ${RN}px);flex:0 0 auto}.pb6-right{width:min(100vw - 16px, ${RN}px);flex:0 0 auto}.pb6-canvas{width:100%!important;height:auto!important;aspect-ratio:${RN}/${NU}}}
  ...
  <div class="pb6">
    <div class="pb6-top"><div style="font-weight:900;font-size:10px;letter-spacing:.15em;color:#00F0FF">PUZZLE BUBBLE • v16 GHOST FIX • 30 SHOTS AD</div><div id="pb6lvl" style="background:#1b1b27;border-radius:16px;padding:4px 8px;font-size:10px;font-weight:800">LVL 1</div></div>
  ...
  function checkAd(){
    if(shotsSinceAd>=SHOTS_PER_AD){
      shotsSinceAd=0;
      if(window.vrAd===0 && !isShooting){
        pend='inter'; window.vrAdType='inter'; window.vrAd=1;
      }
    }
  }
  ...
  function pushCeil(){
    ...
    for(let r=Q-1;r>0;r--){ ng[r]=[...old[r-1]]; if(r%2===1) ng[r][B-1]=null; }
    let nr=Array(B).fill(null);
    for(let c=0;c<B;c++){ if(c===B-1){ nr[c]=null; continue; } if(Math.random()>0.15) nr[c]=Math.floor(Math.random()*colors()); }
    ng[0]=nr;
  ...
  function drawBoard(){
    ...
    // FIX: solo dibujar shooting si isShooting activo, no fantasma después de place
  ...
  function loop(){
    if(isShooting && shooting){
      ...
      for(let r=0;r<Q;r++) for(let c=0;c<B;c++) if(grid[r][c]!==null){ let g=Oe(r,c); if(Math.hypot(g.x-shooting.x,g.y-shooting.y)<WE*0.9){ let p=lu(shooting.x,shooting.y,grid); if(p) place(p.r,p.c,shooting.col); else { isShooting=false; shooting=null; drawBoard(); } return; } }
      drawBoard();
  ...
  function place(r,c,col){
    // FIX CRITICO: limpiar shooting ANTES de cualquier draw para que no quede fantasma
    let pendingCol = col;
    isShooting=false;
    ...
    if(!isConnectedTop(ng,r,c)){ 
      ng[r][c]=null; 
      shot++; shotsSinceAd++; checkAd();
      updateUI(); grid=ng; drawBoard(); return; 
    }
    let conn=vm(ng,r,c);
    if(conn.length>=3){
      // borrar todas incluyendo la nueva - FIX: no dejar fantasma
