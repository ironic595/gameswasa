// game v14 - FIX TILDES + FIX COLOR CHANGE ON CEIL DROP - LITE ONLY
...
@media(max-width:900px){.pb6-wrap{padding:8px}.pb6-body{flex-direction:column;align-items:center;width:100%}.pb6-left{width:min(100vw - 16px, ${RN}px);flex:0 0 auto}.pb6-right{width:min(100vw - 16px, ${RN}px);flex:0 0 auto}.pb6-canvas{width:100%!important;height:auto!important;aspect-ratio:${RN}/${NU}}}
  ...
  <div class="pb6">
    <div class="pb6-top"><div style="font-weight:900;font-size:10px;letter-spacing:.15em;color:#00F0FF">PUZZLE BUBBLE • v14 FIX • CANVAS</div><div id="pb6lvl" style="background:#1b1b27;border-radius:16px;padding:4px 8px;font-size:10px;font-weight:800">LVL 1</div></div>
  ...
  function initGrid(){
    ...
    // FIX: clean odd rows last column
  ...
  function pushCeil(){
    if(isShooting) return; // FIX 1: no bajar techo mientras dispara -> evita tildada
    const old = tu(grid);
    let ng=Array.from({length:Q},()=>Array(B).fill(null));
    for(let r=Q-1;r>0;r--){ 
      ng[r]=[...old[r-1]]; 
      // FIX 2: recortar fila impar para no crear bola fantasma que parece cambio de color
      if(r%2===1) ng[r][B-1]=null;
    }
    let nr=Array(B).fill(null);
    for(let c=0;c<B;c++){ 
      if(c===B-1) { nr[c]=null; continue; } // top row odd? top is 0 even, so allow B cols but...
      if(Math.random()>0.15) nr[c]=Math.floor(Math.random()*colors()); 
    }
    ng[0]=nr;
    // no flotantes: si no está conectado al techo, no borrar ahora, solo verificar game over
    let floating=hm(ng); 
    // no borres flotantes al bajar techo, solo si son desconectados? original los borraba y cambiaba colores visualmente
    // FIX: no borrar flotantes en push, solo check game over
    if(ng[Q-1].some(v=>v!==null)){ lose(); return; }
    grid=ng; shot=0; drawBoard();
  }
  ...
  function loop(){
    if(isShooting && shooting){
      ...
      if(shooting.y<=WE/2+10){ let p=lu(shooting.x,shooting.y,grid); if(p) place(p.r,p.c,shooting.col); else lose(); isShooting=false; shooting=null; drawBoard(); return; }
      for(let r=0;r<Q;r++) for(let c=0;c<B;c++) if(grid[r][c]!==null){ let g=Oe(r,c); if(Math.hypot(g.x-shooting.x,g.y-shooting.y)<WE*0.9){ let p=lu(shooting.x,shooting.y,grid); if(p) place(p.r,p.c,shooting.col); else lose(); isShooting=false; shooting=null; drawBoard(); return; } }
      drawBoard();
  ...
  function place(r,c,col){
    if(grid[r][c]!==null){ let alt=lu(Oe(r,c).x,Oe(r,c).y,grid); if(!alt){ isShooting=false; shooting=null; drawBoard(); return; } r=alt.r; c=alt.c; }
    let ng=tu(grid); ng[r][c]=col;
    if(r%2===1) ng[r][B-1]=null; // FIX parity
    if(!isConnectedTop(ng,r,c)){ cur=nxt; nxt=Math.floor(Math.random()*colors()); isShooting=false; shooting=null; shot++; updateUI(); grid=ng; drawBoard(); return; }
    let conn=vm(ng,r,c);
    if(conn.length>=3){
      ...
      grid=ng; updateUI(); drawBoard();
      if(popped>=target()||grid.flat().every(v=>v===null)){ win(); return; }
    } else { grid=ng; drawBoard(); }
    shot++; if(shot>=need()){ pushCeil(); ceilT=maxT(); shot=0; } cur=nxt; nxt=Math.floor(Math.random()*colors()); isShooting=false; shooting=null; updateUI(); drawBoard(); if(grid[Q-1].some(v=>v!==null)) lose();
  }
  function win(){
    if(ceilIv) clearInterval(ceilIv); isShooting=false; if(rafId) cancelAnimationFrame(rafId);
