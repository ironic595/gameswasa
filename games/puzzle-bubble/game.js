// game.v6.js - LAYOUT FIX: paneles al costado, juego grande
export function init(container, args){
 ...
 .pb6-body{flex:1;display:flex;gap:14px;padding:12px;max-width:1200px;width:100%;margin:0 auto;min-height:0;align-items:flex-start}
 .pb6-left{flex:0 0 ${RN}px;width:${RN}px;background:#0f0f17;border:1px solid rgba(255,255,255,.1);border-radius:20px;overflow:hidden;position:relative;display:flex;flex-direction:column}
 .pb6-board{position:relative;width:${RN}px;height:${NU}px;touch-action:none;background:#0f0f17}
 .pb6-right{flex:1;min-width:260px;max-width:360px;display:flex;flex-direction:column;gap:10px;position:sticky;top:0}
 .pb6-stats{display:grid;grid-template-columns:1fr 1fr;gap:8px}
 .pb6-st{background:rgba(21,21,29,.9);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px}.pb6-st b{font-family:monospace;font-size:16px;display:block}.pb6-st span{font-size:9px;opacity:.5;text-transform:uppercase;letter-spacing:.06em}
 ...
 .pb6-card{background:linear-gradient(180deg,#15151f,#0f0f17);border:1px solid rgba(255,255,255,.12);border-radius:24px;padding:22px;text-align:center;width:min(360px,92vw)}
  @keyframes pop{0%{transform:scale(.7);opacity:0}40%{transform:scale(1.15);opacity:1}100%{transform:scale(1);opacity:1}}
  @media(max-width:900px){.pb6-body{flex-direction:column;align-items:center}.pb6-left{flex:0 0 auto;width:min(100vw - 24px, ${RN}px)}.pb6-right{max-width:520px;width:100%;position:static}}
  </style>
  <div class="pb6" id="pb6">
    <div class="pb6-top"><div style="font-weight:900;font-size:11px;letter-spacing:.18em;color:#00F0FF">PUZZLE BUBBLE • v6 PROGRESIVO • ${RN}x${NU} • FIX LAYOUT</div><div style="display:flex;gap:6px"><div style="background:#1b1b27;border-radius:20px;padding:6px 10px;font-size:10px;font-weight:800" id="pb6lvl">LVL 1</div><div style="background:linear-gradient(135deg,#FFE600,#FF6B2B);color:#000;border-radius:20px;padding:6px 10px;font-size:10px;font-weight:900" id="pb6w">0 WASA</div></div></div>
    <div class="pb6-body">
  ...
  function place(r,c,col){
    ...
    if(conn.length>=3){
      ...
      let floating=hm(ng);
      if(floating.length){ floating.forEach(([rr,cc])=>{ let p=Oe(rr,cc); popAnim(p.x,p.y,ng[rr][cc],8); ng[rr][cc]=null; }); }
  ...
  function lose(){ if(ceilIv) clearInterval(ceilIv); ui.innerHTML=`<div class="pb6-win"><div class="pb6-card" style="background:#1a1012;border-color:rgba(255,43,43,.3)"><div style="width:64px;height:64px;margin:0 auto;border-radius:50%;background:#FF2B2B;display:grid;place-items:center;font-size:28px">✕</div><h2 style="font-size:20px;font-weight:900;margin-top:10px">TECHO ALCANZADO</h2><p style="font-size:12px;opacity:.6;font-family:monospace;margin-top:4px">${popped}/${target()}</p><div style="display:flex;gap:8px;margin-top:16px"><button id="bAg" style="flex:1;height:44px;border-radius:24px;background:#fff;color:#000;font-weight:800">REINTENTAR LVL ${level}</button><button id="bR" style="flex:1;height:44px;border-radius:24px;background:#222;color:#fff;border:1px solid rgba(255,255,255,.15)">RESET</button></div></div></div>`; ui.querySelector('#bAg').onclick=()=>{ ui.innerHTML=''; initGrid(); }; ui.querySelector('#bR').onclick=()=>{ level=1; localStorage.setItem('pb_level',1); ui.innerHTML=''; initGrid(); }; }
