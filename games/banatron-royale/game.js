// games/banatron-royale/game.js - v1.4 - FIX AD BLANCO + MODAL ENCIMA DEL AD
...
.bt-ui{position:absolute;inset:0;pointer-events:none;z-index:10}
...
.bt-hint.hide{display:none !important}
.bt-ad-overlay{position:absolute;inset:0;background:#000000f0;z-index:50;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#c084fc;text-align:center;pointer-events:auto}
.bt-ad-box{width:100%;max-width:480px;background:#000;border-radius:12px;overflow:hidden}
  </style><div class="bt"><canvas id="c"></canvas><div id="ui" class="bt-ui"></div><div id="hint" class="bt-hint">🐍 SNAKE ROYALE • 4 serpientes desafiándose</div>
  <div id="adFixStyle"></div>
  </div>`;
  ...
  // FIX CSS global para el contenedor de anuncios de WASA (el rectangulo blanco)
  const styleFix = document.createElement('style');
  styleFix.textContent = `
    /* El div del ad que se inyecta fuera del juego */
    #wasa-ad-container, .wasa-ad-wrapper, [id*="ad-container"] {
    ...
    #wasa-ad-container iframe, .wasa-ad-wrapper iframe {
      background:#000 !important;
  ...
  function getAliveCount(){ return players.filter(p=>p.alive).length; } function getPlayerAlive(){ return players.find(p=>p.isPlayer)?.alive; } function getKills(){ return players.filter(p=>!p.isPlayer && !p.alive).length; }
  function openAd(t){ 
  ...
  function drawSnake(p){
    ...
    if(!isDead){
      ctx.strokeStyle = p.light; ctx.lineWidth = 1.5; ctx.stroke();
      ctx.shadowColor = p.glow; ctx.shadowBlur = 8; ctx.stroke(); ctx.shadowBlur=0;
    }
    if(isDead){
      ctx.strokeStyle='#4a044e'; ctx.lineWidth=1.5; const eyeOff = headRadius*0.35;
      ctx.beginPath(); ctx.moveTo(hx-eyeOff-3, hy-3); ctx.lineTo(hx-eyeOff+3, hy+3); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(hx-eyeOff-3, hy+3); ctx.lineTo(hx-eyeOff+3, hy-3); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(hx+eyeOff-3, hy-3); ctx.lineTo(hx+eyeOff+3, hy+3); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(hx+eyeOff-3, hy+3); ctx.lineTo(hx+eyeOff+3, hy-3); ctx.stroke();
    } else {
      const dir = p.dir; const fwd = DIRS[dir]; const eyeBaseX = hx + fwd.x*headRadius*0.35; const eyeBaseY = hy + fwd.y*headRadius*0.35; const perpX = -fwd.y, perpY = fwd.x;
      ctx.fillStyle='white'; ctx.beginPath(); ctx.arc(eyeBaseX + perpX*headRadius*0.3, eyeBaseY + perpY*headRadius*0.3, headRadius*0.28, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(eyeBaseX - perpX*headRadius*0.3, eyeBaseY - perpY*headRadius*0.3, headRadius*0.28, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle='black'; ctx.beginPath(); ctx.arc(eyeBaseX + perpX*headRadius*0.3 + fwd.x*2, eyeBaseY + perpY*headRadius*0.3 + fwd.y*2, headRadius*0.12, 0, Math.PI*2); ctx.fill();
  ...
  function renderUI(){
    ...
    // Si hay anuncio reproduciendose (vrAd 1,2,3), NO mostrar nada de juego encima
    if(window.vrAd===1 || window.vrAd===2 || window.vrAd===3){
      ui.innerHTML=`<div style="position:absolute;inset:0;background:#000000f0;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:100;pointer-events:none">
        <div style="color:#c084fc;font-size:12px;letter-spacing:.2em">🐍 ANUNCIO EN CURSO...</div>
        <div style="color:#64748b;font-size:10px;margin-top:6px">El juego está pausado • No toques</div>
      </div>`;
      if(hint) hint.classList.add('hide');
      return;
    }
    ...
    if(state==='menu'){
      ...
      ui.innerHTML=`<div style="position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%, rgba(168,85,247,.25), transparent 60%), linear-gradient(180deg,#0a0a1a,#1a0a2e);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:14px;text-align:center">
        <div style="font-size:10px;letter-spacing:.3em;color:#c084fc">🐍 SNAKE STYLE v1.4 AD FIX • ${hasPass?'PASS X5 💎':''}</div>
        ...
        <div style="margin-top:8px;color:#e9d5ff;font-size:11px">Serpientes con cabeza • Ojos que miran • AD FIX</div>
      ...
      ui.innerHTML=`<div style="position:absolute;inset:0;background:rgba(10,10,26,.92);display:flex;align-items:center;justify-content:center;padding:12px"><div style="background:#1a1030;border:2px solid ${isWin?'#c084fc':'#ef4444'};border-radius:16px;padding:16px;width:100%;max-width:340px;text-align:center">
      ...
      // Mientras carga el ad, pantalla negra total para que no se vea el juego de fondo con el rectangulo blanco
      ui.innerHTML=`<div style="position:absolute;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:100">
        <div style="color:#c084fc;font-weight:900;font-size:14px;letter-spacing:.2em">🐍 CARGANDO ANUNCIO...</div>
        <div style="color:#64748b;font-size:10px;margin-top:8px">Gana X2 si lo ves completo</div>
      </div>`;
      if(hint) hint.classList.add('hide');
    }else if(state==='reward_modal'){
      // SOLO se muestra cuando vrAd===4, nunca durante el ad
      const kills=getKills(); const rew=rewardWithPass(kills)*2;
      ui.innerHTML=`<div style="position:absolute;inset:0;background:rgba(0,0,0,.88);display:flex;align-items:center;justify-content:center;padding:12px;z-index:110"><div style="background:#1a1030;border:1px solid #c084fc;border-radius:12px;padding:14px;width:100%;max-width:280px;text-align:center"><div style="color:#c084fc;font-weight:900">🐍 X2 SNAKE ROYALE! ${hasPass?'💎 X10':''}</div><div style="color:white;font-size:13px;margin-top:6px">${fmt(lastReward)} → ${fmt(rew)} WASA</div><button id="claim2" style="margin-top:10px;width:100%;background:#c084fc;color:#000;font-weight:900;padding:12px;border-radius:999px;border:0">RECLAMAR ${fmt(rew)} 💎</button></div></div>`;
      ui.querySelector('#claim2').onclick=async()=>{ const btn=ui.querySelector('#claim2'); if(btn){ btn.textContent='⏳ VALIDANDO...'; btn.disabled=true; } adClaimed=true; const res=await claimSession(true,true,kills); if(res.ok){ resetRound(1); state='playing'; renderUI(); } };
      if(hint) hint.classList.add('hide');
    }
  }
  ...
  // WATCHER FIX: solo pasar a reward_modal cuando vrAd es 4 y ya NO está en 1,2,3
  let watcher=setInterval(()=>{ 
    if(window.vrAd===4 && _rewardPending && state!=='reward_modal' && state!=='roundover'){
      // doble check: que no siga habiendo skip button visible
      state='reward_modal'; renderUI(); 
    }
    if(window.vrAd===4 && _rewardPending && state==='paused_ad'){
      state='reward_modal'; renderUI();
    }
  },200);
  ...
  function loop(now){
    // Si hay ad reproduciendose, pausar juego y no dibujar serpientes debajo del ad blanco
    if(window.vrAd===1||window.vrAd===2||window.vrAd===3){ 
      // forzar repintar UI para que tape el rectangulo blanco con fondo negro
      raf=requestAnimationFrame(loop); 
      // dibujar fondo negro puro para tapar el rectangulo blanco feo
  ...
  container._cleanup=()=>{ cancelAnimationFrame(raf); clearInterval(watcher); ro.disconnect(); window.vrAd=0; if(styleFix.parentNode) styleFix.parentNode.removeChild(styleFix); };
}
