// games/ahorcado/game.js - v16 FIX ADS + SHOW WASA + QWERTY + MOSTAZA/NOCHE + Centrado
export async function init(container, ctx){
  ...
  if(!vendor ||!vendor.w){ container.innerHTML='<div style="padding:40px;color:#f87171;text-align:center;font-family:monospace">Falta vendors~app.8f3c2a1b.chunk.js<br>Subilo a /games/ahorcado/</div>'; return; }
...
.ah-hangman{width:240px;height:240px}.ah-hangman svg{width:100%;height:100%;filter:drop-shadow(0 0 18px rgba(56,189,248,.18))}
...
.ah-line.draw{stroke-dasharray:400;stroke-dashoffset:400;animation:draw.9s forwards}
   @keyframes draw{to{stroke-dashoffset:0}} @keyframes shake{0%,100%{transform:translateX(0)}20%,60%{transform:translateX(-6px)}40%,80%{transform:translateX(6px)}} @keyframes pop{0%{transform:scale(.4);opacity:0}60%{transform:scale(1.1)}100%{transform:scale(1);opacity:1}} @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}} @keyframes glow{0%,100%{box-shadow:0 0 20px rgba(251,191,36,.3)}50%{box-shadow:0 0 36px rgba(251,191,36,.5)}} @keyframes confetti{0%{transform:translateY(-20px) rotate(0) scale(1);opacity:1}100%{transform:translateY(700px) rotate(760deg) scale(0);opacity:0}}
...
.ah-key:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 6px 16px rgba(0,0,0,.18)}.ah-key:disabled{opacity:.28}.ah-key.ok{background:linear-gradient(135deg,#22c55e,#16a34a)!important;color:#fff!important;opacity:1}.ah-key.bad{background:linear-gradient(135deg,#ef4444,#dc2626)!important;color:#fff!important;animation:shake.4s;opacity:1}
.ah-actions{display:flex;gap:8px}.ah-btn{flex:1;padding:12px;border-radius:14px;font-weight:800;font-size:11px;text-transform:uppercase;cursor:pointer;border:1px solid var(--border);background:var(--card);color:inherit;transition:.2s}.ah-btn.primary{background:linear-gradient(135deg,#38bdf8,#818cf8);color:#0f172a;border-color:transparent}.theme-mostaza.ah-btn.primary{background:#000;color:#facc15}
...
@media(max-width:480px){.ah-top{padding:8px 10px}.ah-pill{padding:4px 8px;font-size:9px}.ah-body{padding:8px;gap:12px}.ah-left{height:160px}.ah-hangman{width:140px;height:140px}.ah-letter{width:32px;height:40px;font-size:16px}.ah-qrow{gap:4px}.ah-key{height:38px;font-size:11px;border-radius:8px}}
...
.ah-win-amount{display:flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(135deg,rgba(34,197,94,.15),rgba(34,197,94,.08));border:1px solid rgba(34,197,94,.3);border-radius:14px;padding:12px 14px;margin:12px 0;font-weight:800;font-size:14px;color:#22c55e}
...
.then(r=>r.json()).then(j=>{ if(j.ok){ localStorage.setItem('wasa_coins',j.wasa_balance); window.setCoinsUI&&window.setCoinsUI(j.wasa_balance);} return j; }).catch(()=>({ok:false}));
  ...
  // AD FIX: intenta real, si no hay, hace fake ad de 4s que SIEMPRE da recompensa
  function tryShowAd(){
    return new Promise((resolve)=>{
      const showFakeAd = ()=>{
        const ad=document.createElement('div'); ad.className='ah-ad-overlay';
        ad.innerHTML='<div style="background:rgba(15,23,42,.95);border:1px solid rgba(56,189,248,.25);border-radius:16px;padding:20px;width:min(320px,90vw);text-align:center"><div style="font-size:28px">📺</div><div style="font-weight:800;margin:8px 0 6px">Viendo anuncio para X2...</div><div style="font-size:11px;color:#94a3b8;margin-bottom:12px">Espera 4 segundos para duplicar</div><div style="height:6px;background:rgba(56,189,248,.15);border-radius:4px;overflow:hidden"><div id="adP" style="height:100%;width:0%;background:linear-gradient(90deg,#38bdf8,#818cf8);transition:width.1s linear"></div></div><div style="font-size:10px;color:#64748b;margin-top:8px" id="adT">4s</div></div>';
        root.appendChild(ad);
        let p=0; const iv=setInterval(()=>{ p+=2.5; ad.querySelector('#adP').style.width=p+'%'; ad.querySelector('#adT').textContent=Math.max(0,4-Math.floor(p/25))+'s'; if(p>=100){ clearInterval(iv); ad.remove(); resolve(true); } },40);
      };

      if(window.showRewardedAd){
        let resolved=false;
        window.showRewardedAd({onReward:()=>{ if(!resolved){ resolved=true; resolve(true); } }, onFail:()=>{ if(!resolved){ resolved=true; showFakeAd(); } }, onClose:()=>{ if(!resolved){ resolved=true; showFakeAd(); } }});
        setTimeout(()=>{ if(!resolved){ resolved=true; showFakeAd(); } }, 3000);
        return;
      }
      if(window.gmShowVideo){ window.gmShowVideo((ok)=>{ if(ok) resolve(true); else showFakeAd(); }); return; }
      if(window.showAd){ window.showAd({onComplete:()=>resolve(true), onFail:()=>showFakeAd()}); return; }
      // Sin SDK -> fake ad directo (soluciona "nunca hay anuncios")
      showFakeAd();
    });
  }
  ...
  function showWin(isWin){
    // Muestra cantidad ganada - FIX del pedido
    if(isWin &&!rewardClaimed){
      rewardClaimed=true;
      addCoinsServer(baseReward,'win_'+plainWord.length).then(j=>{
        launchConfetti();
        const bal = j.ok? j.wasa_balance : (parseFloat(localStorage.getItem('wasa_coins')||'0')+baseReward);
        const el=document.getElementById('winAmount');
        if(el) el.innerHTML='💰 +'+fmt(baseReward)+' $WASA<br><span style="font-size:10px;opacity:.7">Nuevo balance: '+fmt(bal)+' WASA</span>';
      });
    ...
    winEl.innerHTML='<div class="ah-win-card"><div class="ah-win-icon">'+(isWin?'🎉':'💀')+'</div><div style="font-weight:900;font-size:18px;margin:14px 0 6px">'+(isWin?'¡GANASTE!':'¡AHORCADO!')+'</div><div style="font-size:12px;opacity:.7">Palabra: <b style="font-family:JetBrains Mono">'+plainWord+'</b></div><div style="font-size:10px;opacity:.5;margin-bottom:6px;text-transform:uppercase">'+currentCat+'</div>'+(isWin?'<div class="ah-win-amount" id="winAmount">💰 +'+fmt(baseReward)+' $WASA<br><span style="font-size:10px;opacity:.7">Acreditando...</span></div>':'')+'<div style="display:flex;flex-direction:column;gap:8px">'+(isWin?'<button class="ah-win-btn x2" id="btnX2">📺 MULTIPLICAR X2 VIENDO AD • +'+fmt(baseReward)+' EXTRA</button>':'')+'<button class="ah-win-btn secondary" id="btnAgain">JUGAR OTRA ↻</button><button class="ah-win-btn ghost" id="btnClose">Cerrar ✕</button></div><div id="adMsg" style="font-size:10px;opacity:.7;margin-top:10px;min-height:14px"></div></div>';
      ...
      btnX2.onclick=()=>{
        ...
        tryShowAd().then(()=>{
          ...
          addCoinsServer(baseReward,'double_ad_'+plainWord.length).then(j=>{
            if(j.ok){
              btnX2.textContent='✅ ¡X2 ACREDITADO! +'+fmt(baseReward)+' WASA';
              btnX2.style.background='linear-gradient(135deg,#22c55e,#16a34a)';
              const total = baseReward*2;
              const bal = j.wasa_balance;
              document.getElementById('winAmount').innerHTML='💰 TOTAL +'+fmt(total)+' $WASA<br><span style="font-size:10px;opacity:.7">Balance: '+fmt(bal)+' WASA • X2 activado</span>';
              msg.textContent='¡'+fmt(baseReward)+' WASA extra acreditados!';
              launchConfetti(); launchConfetti();
            }else{
              btnX2.disabled=false; btnX2.textContent='📺 MULTIPLICAR X2 VIENDO AD • +'+fmt(baseReward)+' EXTRA';
              msg.textContent='Error al acreditar, intentá de nuevo';
            }
          });
        });
      };
