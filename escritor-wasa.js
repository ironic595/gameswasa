...
export async function crearWasaPassFinal({codeId, apodo, email, tx_hash, multiplier=5}){
  ...
  const nickRect={x:Math.floor(W*0.143), y:Math.floor(H*0.8549), w:Math.floor(W*0.49), h:Math.floor(H*0.0459)}; // corrido +3.3% a la derecha para centrar entre borde y QR - fix tu captura ANDRYZEN5600
  ...
  const qrRect={x:Math.floor(W*0.7335), y:Math.floor(H*0.832), w:qrSize, h:qrSize}; // subido - no pisa 2026
  let qrImg;
  if(window.QRCode){
