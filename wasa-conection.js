// wasa-conection.js v4 - icono gris si no conectado, amarillo si conectado + menu chiquito
const WASA_CONFIG = {
...
function updateWalletUI(){
  ...
  if(userBtn){
    if(isLogged()){
      ...
      userBtn.className = 'user-btn connected'; // amarillo
      userBtn.title = connectedNickname || connectedEmail || connectedWallet || 'Mi perfil';
    }else{
      userBtn.textContent = '👤';
      userBtn.className = 'user-btn disconnected'; // gris
      userBtn.title = 'Iniciar sesión';
    }
  }
  // si se deslogea, cerrar menu
...
async function doLogin(){
  ...
  try{
    ...
    updateWalletUI(); showAuthStatus('✅ Login OK','status-ok'); setTimeout(()=>{ closeAuthEmail(); },600);
...
async function doRegister(){
  ...
  try{
    ...
    updateWalletUI(); showAuthStatus('✅ Cuenta creada','status-ok',true); setTimeout(()=>{ closeAuthEmail(); },600);
...
async function requestPasswordReset(){
  ...
  try{
    ...
    const j=await r.json(); show(j.msg||'Código enviado','status-ok');
...
async function verifyPasswordReset(){
  ...
  const el=document.getElementById('forgotStatus');
  const show=(m,c)=>{ if(el){ el.textContent=m; el.className='status-box '+c; el.style.display='block'; } };
  if(code.length!==6) return show('Código 6 dígitos','status-err');
  if(newPass.length<6) return show('Mín 6','status-err');
  ...
  try{
    ...
    const j=await r.json(); if(!j.ok) throw new Error(j.error);
    show('✅ Contraseña cambiada','status-ok'); setTimeout(()=>{ closeForgot(); switchAuthTab('login'); },1000);
  }catch(e){ show('❌ '+e.message,'status-err'); }
}

// FIX WALLETS - cada una su página correcta
async function connectWith(type){
  const isMobile=/Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
  const currentUrl=window.location.href; const domain=currentUrl.replace('https://','').replace('http://','').split('/')[0]; const path=window.location.pathname;
  try{
    if(window.ethereum){
    ...
    if(isMobile){
      ...
      if(type==='rabby' || type==='okx'){ window.location.href=`https://metamask.app.link/dapp/${domain}${path}`; return; }
    }
...
async function buyWasa(amt){
  ...
  try{
    ...
    try{ await fetch(WASA_CONFIG.WORKER_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'buy_wasa',wallet:connectedWallet,nickname:connectedNickname||'',email:connectedEmail||'',wasa_amount:amt,usdt_amount:usdt,network:'BSC',tx_hash:txHash})}); }catch(e){}
...
// NUEVO - USER MENU CHIQUITO + PERFIL
...
function openUserMenu(){
  ...
  menu.classList.add('open'); userMenuOpen=true;
}
function closeUserMenu(){
  const menu=document.getElementById('userDropdown'); if(menu) menu.classList.remove('open'); userMenuOpen=false;
}
function openUserProfile(){
  ...
  document.getElementById('profileCoins').textContent = (typeof fmtWASA!=='undefined'? fmtWASA(localStorage.getItem('wasa_coins')||0) : localStorage.getItem('wasa_coins')||'0') + ' $WASA';
  ...
  document.getElementById('profileNickname').value = connectedNickname || localStorage.getItem('wasa_nick') || '';
...
async function saveProfile(){
  ...
  try{ await fetch(WASA_CONFIG.WORKER_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'update_profile',email:connectedEmail,nickname:nick,wallet:connectedWallet})}); }catch(e){}
...
function logoutUser(){
  ...
  connectedWallet=null; connectedNickname=null; connectedEmail=null; updateWalletUI();
}

// clicks fuera cierran el menu
...
window.addEventListener('DOMContentLoaded', async()=>{
  ...
  if(window.ethereum){ window.ethereum.request({method:'eth_accounts'}).then(a=>{ if(a[0]){ connectedWallet=a[0]; localStorage.setItem('wasa_wallet',a[0]); updateWalletUI(); } }).catch(()=>{}); }
});
