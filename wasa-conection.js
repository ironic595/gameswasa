// wasa-conection.js FINAL - con typo original - para games.wasa.chat
// Funciona con el index que ya tiene tabs: tabLogin/tabRegister/loginForm/registerForm/forgotBox
// D1: users, purchases, config, password_resets
const WASA_CONFIG = {
...
function openAuthEmail(tab){
  if(tab) authTab=tab;
  closeWalletChooser();
  const el=document.getElementById('authModal'); if(el) el.classList.add('open');
...
async function doLogin(){
  ...
  try{
    ...
    connectedNickname=j.nickname||null; updateWalletUI();
    showAuthStatus('✅ Login OK','status-ok'); setTimeout(()=>{ closeAuthEmail(); openWasaBuy(); },700);
...
async function doRegister(){
  ...
  try{
    ...
    const j=await r.json(); if(!j.ok) throw new Error(j.error);
    localStorage.setItem('wasa_email', j.email); if(nick) localStorage.setItem('wasa_nick', nick);
    showAuthStatus('✅ Cuenta creada','status-ok',true); setTimeout(()=>{ closeAuthEmail(); openWasaBuy(); },700);
...
function openForgot(){
  ...
  const f1=document.getElementById('forgotStep1'), f2=document.getElementById('forgotStep2');
...
async function requestPasswordReset(){
  ...
  try{
    ...
    const j=await r.json();
    show(j.msg || 'Código enviado a '+email,'status-ok');
...
async function verifyPasswordReset(){
  ...
  if(newPass.length<6) return show('Nueva pass mín 6','status-err');
  ...
  try{
    ...
    const j=await r.json(); if(!j.ok) throw new Error(j.error);
    show('✅ Contraseña cambiada! Entrá con la nueva','status-ok');
    setTimeout(()=>{ closeForgot(); switchAuthTab('login'); document.getElementById('authEmail').value=email; },1200);
  }catch(e){ show('❌ '+e.message,'status-err'); }
}
...
async function connectWith(type){
  ...
  const currentUrl=window.location.href; const domain=currentUrl.replace('https://','').replace('http://','').split('/')[0];
  try{
    if(window.ethereum){
...
async function buyWasa(wasaAmount){
  const usdtAmount=WASA_CONFIG.PRICES[wasaAmount]; const nick=document.getElementById('nicknameInput')?.value.trim()||''; const statusEl=document.getElementById('buyStatus');
  const show=(m,c)=>{ if(statusEl){ statusEl.textContent=m; statusEl.className='status-box '+c; statusEl.style.display='block'; } };
  ...
  try{
    ...
    try{ await fetch(WASA_CONFIG.WORKER_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'buy_wasa',wallet:connectedWallet,nickname:nick||connectedNickname||'',email:localStorage.getItem('wasa_email')||'',wasa_amount:wasaAmount,usdt_amount:usdtAmount,network:'BSC',tx_hash:txHash})}); }catch(e){ console.warn(e); }
